...
if($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if (count($errors) > 0) {
		...
		$add_column_created_at = "ALTER TABLE `electives` ADD `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP AFTER `lecturer`;";
		$conn->query($add_column_created_at);
		$insertElectiveQuery = $conn->prepare("INSERT INTO `electives`(`title`, `description`, `lecturer`) VALUES (:subject, :description, :lecturer)");
